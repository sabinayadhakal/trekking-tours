...
export default function AdminDashboard() {
  ...
  React.useEffect(() => {
    ...
    void Promise.all(["freeTours", "multiDayTours", "dayHikings", "daySightseeings", "mountainFlights", "jungleSafaris"].map((name) => loadManagedServices(name as Parameters<typeof loadManagedServices>[0], { allowFallback: false }))).then(([freeTours, multiDayTours, dayHikings, daySightseeings, mountainFlights, jungleSafaris]) => setServiceCounts({ freeTours: freeTours.services.length, multiDayTours: multiDayTours.services.length, dayHikings: dayHikings.services.length, daySightseeings: daySightseeings.services.length, mountainFlights: mountainFlights.services.length, jungleSafaris: jungleSafaris.services.length }));
