...
import { ArrowDown, ArrowUp, Download, Instagram, Plus, Save, Trash2, Youtube } from "lucide-react";
import { Button } from "@/components/ui/button";
import {
  importHardcodedSocialMediaContent,
  loadSocialMediaContent,
  saveSocialMediaContent,
} from "@/lib/firebase/social-media-repository";
import {
  getYouTubeThumbnail,
  InstagramPost,
  SOCIAL_MEDIA_EMPTY,
...
export default function SocialMediaAdminPage() {
  ...
  React.useEffect(() => {
    void loadSocialMediaContent({ allowFallback: false }).then(({ content, source }) => {
      setContent(content);
      if (source === "unavailable") setMessage("No social media document found in Firebase. Import the current website content or start adding items.");
      setReady(true);
    });
  }, []);
  ...
  const save = async () => {
    ...
    if (hasInvalidVideo || hasInvalidPost) {
      setMessage("Every item needs a title and URL before saving.");
    ...
    setSaving(false);
    setMessage(result.source === "firestore" ? "Social media content saved to Firebase. Home and blog now use these items." : result.error || "Could not save to Firebase.");
  };

  const importCurrent = async () => {
    setSaving(true);
    const result = await importHardcodedSocialMediaContent();
    setSaving(false);
    if (result.source === "firestore") {
      setContent(structuredClone(SOCIAL_MEDIA_FALLBACK));
      setMessage("Current hardcoded social media content imported to Firebase.");
  ...
  return <div className="px-5 py-8 sm:px-8 sm:py-10 lg:px-12">
    <div className="flex flex-col gap-4 border-b border-[#d8cec0] pb-6 sm:flex-row sm:items-end sm:justify-between">
      ...
      <div className="flex flex-wrap gap-2"><Button type="button" variant="outline" disabled={!ready || saving} onClick={() => void importCurrent()} className="border-[#cf6943] text-[#cf6943]"><Download className="mr-2 h-4 w-4" />Import current content</Button><Button type="button" disabled={!ready || saving} onClick={() => void save()} className="bg-[#cf6943] text-white"><Save className="mr-2 h-4 w-4" />{saving ? "Saving…" : "Save changes"}</Button></div>
    </div>
    ...
    <section className="mt-6 rounded-xl border border-[#d8cec0] bg-[#f7f2e9] p-5 sm:p-7">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="flex items-center gap-2 font-serif text-2xl"><Youtube className="h-6 w-6 text-red-600" />YouTube videos</h2><p className="mt-1 text-sm text-[#66706d]">Add videos and control their display order.</p></div><Button type="button" variant="outline" onClick={() => set("youtubeVideos", [...content.youtubeVideos, { title: "", url: "" }])} className="border-red-300 text-red-700"><Plus className="mr-2 h-4 w-4" />Add video</Button></div>
    ...
    <section className="mt-6 rounded-xl border border-[#d8cec0] bg-[#f7f2e9] p-5 sm:p-7">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="flex items-center gap-2 font-serif text-2xl"><Instagram className="h-6 w-6 text-[#833AB4]" />Instagram posts</h2><p className="mt-1 text-sm text-[#66706d]">Add reels or posts and control their display order.</p></div><Button type="button" variant="outline" onClick={() => set("instagramPosts", [...content.instagramPosts, { title: "", url: "", type: "reel" }])} className="border-[#cf6943] text-[#cf6943]"><Plus className="mr-2 h-4 w-4" />Add Instagram item</Button></div>
