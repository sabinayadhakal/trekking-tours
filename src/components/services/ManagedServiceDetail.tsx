...
export default function ManagedServiceDetail({
  ...
  const fallback = MANAGED_SERVICES_FALLBACK[collection].find(matchesRoute);
  const [service, setService] = React.useState<ManagedService | undefined>(
    fallback,
  );
  const [expandedDays, setExpandedDays] = React.useState<number[]>([1]);
  const refresh = React.useCallback(() => {
    void loadManagedServices(collection).then(({ services }) =>
      setService(services.find(matchesRoute)),
    );
  }, [collection, matchesRoute]);
  React.useEffect(() => {
    refresh();
    const onUpdate = (event: Event) => {
      if ((event as CustomEvent).detail === collection) refresh();
    };
  ...
  if (!service || !service.published)
    return (
      <main className="min-h-[60vh] bg-[#f2ede4] px-5 py-24 text-center">
        <h1 className="font-serif text-4xl text-[#14383b]">
        ...
        <Link
          ...
          className="mt-5 inline-block text-sm font-bold text-[#cf6943]"
        >
          Return to services
        </Link>
      </main>
  ...
  return (
    <>
      <Head>
        <title>{service.name} | Himkala Adventure</title>
        <meta
          name="description"
        ...
        <link
          rel="canonical"
          href={`https://www.himkalaadventure.com${service.link}`}
        />
      </Head>
                  ...
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody>
                      {table.rows.map((row) => (
                        <tr
                          key={row.id}
                          className="border-t border-[#d8cec0]/50"
                        >
                          {table.columns.map((_, index) => (
                            <td
                              key={index}
          ...
          <section className="bg-[#e4d8c8] px-4 py-12 sm:px-5 sm:py-16 md:px-8 lg:py-20">
            <div className="mx-auto max-w-[880px] text-center">
              ...
              <div className="flex flex-wrap items-center justify-center gap-3">
                <Link
                  href={`/contact?trek=${encodeURIComponent(service.name)}`}
                >
